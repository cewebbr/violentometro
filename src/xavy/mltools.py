...
"""
Helpful auxiliary functions for Machine Learning projects
...
def svc_gamma(X, val_type='scale'):
    """
    Compute the value of the SVC parameter `gamma`, given the data `X`.
    
    Parameters
    ----------
    X : 2D array or sparse matrix
        SVC input data. The rows are instances and the columns are features.
    val_type : str
        Either 'scale': 1.0 / (n_features * X.var()) or 'auto':
        1.0 / n_features.
    
    Returns
    -------
    gamma : float
        The SVC gamma parameter.
    """
...
class ParamGridGenerator():
    ...
    def generate_rfc(self):
        self.random_forest_classifier = {'{}n_estimators'.format(self.prefix): [20, 50, 100, 200], 
                                          '{}max_depth'.format(self.prefix): [2, 4, 8, 16, 32],
                                          '{}min_samples_split'.format(self.prefix): [2, 4, 8, 16, 32],
                                          '{}max_features'.format(self.prefix): ['sqrt', 'log2', None],
                                          '{}class_weight'.format(self.prefix): ['balanced', 'balanced_subsample', None],
                                          '{}max_samples'.format(self.prefix): [0.1, 0.2, 0.4, 0.8, 1.0]
                                          }
        return self.random_forest_classifier
    
    def generate_vec(self):
        self.count_vectorizer = {'{}strip_accents'.format(self.prefix): ['unicode', None],
                                 '{}lowercase'.format(self.prefix): [True, False],
                                 '{}stop_words'.format(self.prefix): [None, ['ao']],
                                 '{}ngram_range'.format(self.prefix): [(1,1), (1,2)],
                                 '{}max_df'.format(self.prefix): [1.0, 0.99, 0.97, 0.90, 0.70, 0.30],
                                 '{}min_df'.format(self.prefix): [1, 2],
                                 '{}binary'.format(self.prefix): [True, False],
                                 '{}max_features'.format(self.prefix): [None, 100, 300, 1000, 3000, 10000]
                                 }
        return self.count_vectorizer
                                  
    def generate_svc(self):
        self.svc = {'{}C'.format(self.prefix): [1e-3, 1e-2, 1e-1, 1.0, 1e1, 1e2, 1e3],
                    '{}kernel'.format(self.prefix): ['linear', 'poly', 'rfb', 'sigmoid'],
                    '{}gamma'.format(self.prefix): ['scale', 'auto'],
                    '{}coef0'.format(self.prefix): [-1, 0, 1],
                    '{}class_weight'.format(self.prefix): ['balanced', None],
                    '{}degree'.format(self.prefix): [2, 3, 4, 5]
                    }
        return self.svc
...
def print_cv_scores(estimator, X, y, scorers, cv=5):
    ...
    cv : int, None or False
        How many cross-validation splits
        to do. If False, compute the scores over
        the while X and y, no CV.
    """
    for scorer in scorers:
        
        if cv is False:
            scorer = get_scorer(scorer)
            mean, dev = scorer(estimator, X, y), 0
            dev_mean  = 0
